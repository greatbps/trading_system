...
class MenuHandlers:
    ...
    async def _comprehensive_analysis(self) -> bool:
        """종합 분석 (5개 영역 통합) - 서브메뉴 및 모니터링 추가 기능 포함"""
        ...
        while True:
            try:
                ...
                console.print("[yellow]ℹ️ 실시간 종합 분석 실행 중...[/yellow]")
                
                # Assuming analysis_handlers exists and works
                analysis_results = await self.system.analysis_handlers.run_analysis_for_strategy(strategy_name)
                
                if not analysis_results:
                    console.print("[red]❌ 분석 결과가 없습니다.[/red]")
                else:
                    console.print(f"[green]✅ 실시간 분석 완료: {len(analysis_results)}개 종목[/green]")
                    
                    # 분석 결과를 테이블로 표시
                    self._display_analysis_results_table(analysis_results, strategy_name)
                    
                    if Confirm.ask("\n[bold cyan]분석 결과에서 추천된 종목을 자동매매 모니터링에 추가하시겠습니까?[/bold cyan]"):
                        await self._add_recommendations_to_monitoring(analysis_results, strategy_name)
...
최근 뉴스 ({len(news_result.get('articles', []))}건):
    ...
    async def _basic_supply_demand_analysis(self) -> bool:
        ...
        try:
            ...
            symbols = [s.strip() for s in symbols_input.split(',') if s.strip()] if symbols_input else ["005930", "000660", "035420"]
            results = []
            for symbol in symbols:
                try:
                    stock_data = await self.system.data_collector.get_stock_data(symbol)
