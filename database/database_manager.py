...
# 절대 임포트로 변경
try:
    from database.models import Base, Stock, MarketData, AnalysisResult, Trade, TradeExecution, SystemLog, TradingSession, FilterHistory, TradeHistory
except ImportError:
    # 상대 임포트 시도
    try:
    ...
    async def save_backtest_results(self, results: Dict[str, Any]) -> bool:
        ...
        try:
            async with self.get_async_session() as session:
                ...
                backtest_record = BacktestResults(
                    ...
                    results_data=json.loads(json.dumps(results, default=self._datetime_serializer))  # datetime 직렬화 후 저장
                )
        ...
        try:
            async with self.get_async_session() as session:
                result = await session.execute(
                    text("""
                    INSERT INTO trades 
                    (stock_id, trade_type, order_type, order_price, order_quantity, 
                     executed_price, executed_quantity, order_status, commission, 
                     order_time, execution_time, strategy_name)
                    VALUES (:stock_id, :trade_type, :order_type, :order_price, :order_quantity,
                            :executed_price, :executed_quantity, :order_status, :commission,
                            :order_time, :execution_time, :strategy_name)
                    RETURNING id
                    """),
    ...
    async def get_database_stats(self) -> Dict:
        ...
        try:
            async with self.get_async_session() as session:
                ...
                # 각 테이블별 레코드 수 조회
                tables = ['stocks', 'price_data', 'analysis_results', 'trades', 'trade_history', 'news_data', 'system_logs']
