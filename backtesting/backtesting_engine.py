...
# Removed direct model imports here to prevent potential circular dependency issues
# from database.models import AnalysisResult, Trade, Stock, MarketData, AnalysisResult
...
class BacktestingEngine:
    ...
    async def _get_historical_analysis(self, symbol: str, date: datetime) -> Optional[Dict]:
        """과거 분석 데이터 조회 (데이터베이스에서 가져옴)"""
        from database.models import Stock, MarketData, AnalysisResult # Import inside function
        try:
            if not self.db_manager:
                self.logger.warning("데이터베이스 매니저가 초기화되지 않아 과거 분석 데이터를 가져올 수 없습니다.")
                return None

            with self.db_manager.get_session() as session:
                # 1. Stock 정보 가져오기
                stock = Stock.get_by_symbol(session, symbol)
                if not stock:
                    self.logger.debug(f"DB에 {symbol} 종목 정보 없음")
                    return None
                self.logger.debug(f"DB에서 {symbol} 종목 정보 찾음: {stock.name}")
                ...
                if not analysis_result:
                    self.logger.debug(f"DB에 {symbol} {date.strftime('%Y-%m-%d')} 분석 결과 없음")
                else:
            ...
            self.logger.error(f"과거 분석 데이터 조회 오류: {e}")
            return None

    async def _generate_historical_chart_data(self, symbol: str, current_date: datetime, periods: int = 60) -> List[Dict]:
        """과거 차트 데이터 생성 (데이터베이스에서 가져옴)"""
        from database.models import MarketData # Import inside function
        try:
            if not self.db_manager:
                self.logger.warning("데이터베이스 매니저가 초기화되지 않아 과거 차트 데이터를 가져올 수 없습니다.")
                return []

            with self.db_manager.get_session() as session:
                # periods 만큼의 과거 일봉 데이터 조회
                start_date = current_date - timedelta(days=periods * 1.5) # 여유있게 기간 설정
                self.logger.debug(f"DB에서 {symbol} {start_date.strftime('%Y-%m-%d')} ~ {current_date.strftime('%Y-%m-%d')} 차트 데이터 조회 중...")
                ohlcv_data = MarketData.get_ohlcv_data(session, symbol, '1d', start_date, current_date)
                ...
                ohlcv_data.sort(key=lambda x: x.datetime)

                chart_data = []
                for data_point in ohlcv_data:
                    chart_data.append({
                        'date': data_point.datetime.strftime('%Y-%m-%d'),
    ...
    async def _save_backtest_result(self, result: BacktestResult) -> bool:
        ...
        try:
            ...
            if hasattr(self, 'db_manager') and self.db_manager:
                return await self.db_manager.save_backtest_results(result_dict)
            else:
                self.logger.warning("데이터베이스 매니저가 초기화되지 않음 - 결과 저장 생략")
                return True
        except Exception as e:
