...
class MLDiagnostics:
    ...
    def _display_target_analysis(self, analysis: Dict, target_name: str) -> None:
        ...
        table = Table(title=f"[TARGET] 타겟 변수 '{target_name}' 분석 결과")
    ...
    def _display_feature_quality_analysis(self, analysis: Dict) -> None:
        ...
        table = Table(title="[SEARCH] 특성 데이터 품질 분석")
...
품질 비율: {len(analysis["high_quality_features"]) / analysis["total_features"]:.1%}
        ...
        self.console.print(Panel(summary_text.strip(), title="[DATA] 특성 품질 요약", border_style="blue"))
        ...
        if analysis["problematic_features"]:
            ...
            self.console.print(Panel(problem_text.strip(), title="[WARN] 문제 특성 상세", border_style="red"))
    ...
    def _display_comprehensive_diagnosis(self, diagnosis: Dict) -> None:
        ...
        # 데이터 크기 분석 표시
        table = Table(title="[SIZE] 데이터 크기 분석")
        table.add_column("지표", style="cyan")
        table.add_column("값", style="magenta")
        ...
        if recommendations:
            ...
            self.console.print(Panel(rec_text, title="[IDEA] 개선 권장사항", border_style="blue"))
