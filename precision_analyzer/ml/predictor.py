...
class StockMLPredictor:
    ...
    def train_model(
        ...
        Args:
            ...
            tune_hyperparams: Whether to perform hyperparameter tuning with Optuna
        ...
        try:
            ...
            if tune_hyperparams:
                logger.info("⚙️ Starting hyperparameter tuning with Optuna...")
                best_params = self.model.tune_hyperparameters(X, y, n_trials=50) # 50 trials for speed
                self.model.model_params.update(best_params)
                logger.info("✅ Hyperparameter tuning finished. Using best parameters for training.")
            # --- End of NEW ---

            # --- NEW: SMOTE Activation ---
            # Always apply SMOTE if use_smote is True, regardless of diagnostics
            if use_smote:
                logger.info("[BALANCE] Applying SMOTE to handle class imbalance...")
                ...
                # When using SMOTE, it's often better to disable class_weight
                use_class_weights = False
            # --- End of NEW ---

            # Run diagnostics if enabled (after potential SMOTE)
            ...
            try:
                ...
                logger.warning(f"[WARN] SHAP explainer initialization failed: {e}")
                self.explainer = None
            ...
            logger.error(f"[ERR] Error training model: {e}")
            raise
    ...
    def predict(self, stock_data: pd.DataFrame, indicators: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # Ensure only numeric feature columns are used for prediction
            ...
            feature_df = self.feature_engineer._handle_missing_values(feature_df, self.feature_names)
            
            # Only drop rows with too many missing values (>50% of features)
            threshold = len(feature_df.columns) * 0.5
            feature_df = feature_df.dropna(thresh=threshold)
            
            # Fill remaining NaN values with 0 as last resort
...
Validation AUC: {self.training_metrics.get("val_auc", 0):.3f}
    ...
    def display_feature_importance(self, prediction_result: Dict[str, Any], top_n: int = 10) -> None:
        ...
        if not prediction_result["shap_available"] or self.explainer is None:
            self.console.print("[yellow][WARN]  SHAP analysis not available[/yellow]")
            return
