...
# ... (The rest of the file remains the same)
    ...
    async def get_ohlcv_data(self, symbol: str, period: str = "D", count: int = 100) -> List[OHLCVData]:
        ...
        Returns:
            List[OHLCVData]: OHLCV 데이터 리스트
        """
        try:
            if not symbol or len(symbol) != 6 or not symbol.isdigit():
                raise KISDataValidationError(f"Invalid symbol format: {symbol}")
            
            self.logger.debug(f"Fetching OHLCV data for {symbol}, period={period}, count={count}")
            ...
            output = result.get('output2', [])
            if not output:
                self.logger.warning(f"No OHLCV data returned for symbol {symbol}")
                return []
            ...
            for item in output[:count]:  # 최대 count개까지만
                try:
                    ...
                    self.logger.debug(f"Failed to parse OHLCV item for {symbol}: {item_error}")
                    continue
            
            # 날짜순 정렬 (최신순)
            ohlcv_data.sort(key=lambda x: x.datetime, reverse=True)
            
            self.logger.debug(f"Successfully fetched {len(ohlcv_data)} OHLCV records for {symbol}")
            return ohlcv_data
...
# ... (The rest of the file remains the same)
    ...
    async def cleanup(self):
        """리소스 정리 - aiohttp 세션 정리"""
        try:
            if self.session and not self.session.closed:
                await self.session.close()
                self.logger.info("✅ KisCollector aiohttp 세션 정리 완료")
            self.session = None
            self.status = APIStatus.DISCONNECTED
        except Exception as e:
            self.logger.error(f"❌ KisCollector 정리 중 오류: {e}")
