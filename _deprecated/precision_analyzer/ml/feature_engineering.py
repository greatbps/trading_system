...
class FeatureEngineer:
    ...
    def select_features(self, df: pd.DataFrame, target_col: str, n_features: int = 15) -> pd.DataFrame:
        """
        ...
        Returns:
            DataFrame with only the selected best features and the target column.
        """
        try:
            X = df.drop(columns=[target_col])
            y = df[target_col]

            # --- FIX for SelectKBest dtype error ---
            # Select only numeric columns for feature selection
            numeric_cols = X.select_dtypes(include=np.number).columns.tolist()
            if not numeric_cols:
                logger.warning("No numeric features to select from. Skipping feature selection.")
            ...
            logger.info(f"Selected {len(self.selected_feature_names)} best features: {self.selected_feature_names}")

            # Create a new DataFrame with the selected features and the target
            selected_df = pd.DataFrame(X_new, columns=self.selected_feature_names, index=df.index)
            ...
            self.selected_feature_names = df.drop(columns=[target_col]).columns.tolist()
            return df
    ...
    def create_target_variable(
        ...
        try:
            ...
            # Calculate future maximum price over the next 'prediction_days'
            df['future_max_price'] = df['high'].rolling(window=prediction_days).max().shift(-prediction_days)
            
            # Calculate the percentage change based on the future max price
            price_change_pct = (df['future_max_price'] - df['close']) / df['close']

            if target_type == "binary":
                # 1 if the max price in the future window is >= threshold, 0 otherwise
    ...
    def prepare_features(self, stock_data: pd.DataFrame, indicators: Dict) -> Tuple[pd.DataFrame, List[str]]:
        ...
        try:
            ...
            # Create target variable with new definition
            df = self.create_target_variable(df, target_type="binary", prediction_days=3, threshold_pct=0.02)

            # Remove non-feature columns and identify feature columns
            non_feature_cols = ["open", "high", "low", "close", "volume", "target", "date", "datetime", "future_max_price"]
            ...
            # --- REORDERED LOGIC ---
            # 1. Convert all feature columns to numeric first
            for col in feature_cols:
                df[col] = pd.to_numeric(df[col], errors='coerce')

            # 2. Handle missing values using the imputer
            df = self._handle_missing_values(df, feature_cols + ['target'])

            # 3. Now, perform feature selection on the clean data
            df = self.select_features(df, target_col='target', n_features=15)
            # --- End of REORDERED LOGIC ---

            # Update feature_names to only include the selected ones
            self.feature_names = [col for col in df.columns if col != 'target']

            logger.info(f"Prepared {len(self.feature_names)} features for ML training")
    ...
    def _handle_missing_values(self, df: pd.DataFrame, feature_cols: List[str]) -> pd.DataFrame:
        ...
        # Ensure the imputer is fitted on a valid subset of data
        valid_data_for_imputation = df_copy[feature_cols].dropna()
        if valid_data_for_imputation.empty:
            logger.warning("No valid data to fit the imputer. Returning original dataframe.")
            return df_copy

        imputer.fit(valid_data_for_imputation)

        # Transform the data and create a new DataFrame with correct index and columns
        imputed_data = imputer.transform(df_copy[feature_cols])
        df_copy[feature_cols] = pd.DataFrame(imputed_data, index=df_copy.index, columns=feature_cols)
        
        # Drop rows if any NaNs still remain (should be rare)
        initial_rows = len(df_copy)
        df_copy = df_copy.dropna()
        final_rows = len(df_copy)

        if initial_rows != final_rows:
            logger.warning(f"Dropped {initial_rows - final_rows} rows due to remaining NaNs after imputation.")
