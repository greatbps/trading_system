...
class StockPredictor:
    ...
    def _time_series_split(self, X: pd.DataFrame, y: pd.Series, validation_split: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
        """Splits data into training and validation sets while preserving time order."""
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, validation_split: float = 0.2) -> Dict[str, float]:
        ...
        try:
            ...
            X_train, X_val, y_train, y_val = self._time_series_split(X, y, validation_split)
            ...
            if len(y_val.unique()) < 2:
                logger.warning("[WARN] Validation set contains only one class. Metrics may be unreliable.")

            if len(y_train.unique()) < 2:
                logger.error("[ERR] Training set contains only one class. Cannot train binary classifier.")
            ...
            if val_pred_proba.shape[1] == 2:
                ...
                logger.warning("[WARN] Model prediction probabilities unexpected shape")
            ...
            try:
                ...
                logger.warning(f"[WARN] Error calculating precision/recall/f1: {e}")
            ...
            try:
                if len(y_val.unique()) == 2:
                    ...
                    metrics["val_auc"] = 0.0
                    logger.warning("[WARN] Cannot calculate AUC: validation set doesn't have both classes")
            except Exception as e:
                logger.warning(f"[WARN] Error calculating AUC: {e}")
                metrics["val_auc"] = 0.0
            ...
            if metrics["val_precision"] == 0 and metrics["val_recall"] == 0:
                logger.error("[ERR] Critical: Both precision and recall are 0. Model may be predicting only one class.")
    ...
    def tune_hyperparameters(self, X: pd.DataFrame, y: pd.Series, n_trials: int = 50) -> Dict[str, Any]:
        ...
        logger.info(f"Best trial for hyperparameter tuning: {study.best_trial.value}")
        logger.info(f"Best params: {study.best_params}")

        # Update model_params with the best found parameters
