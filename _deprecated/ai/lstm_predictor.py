...
# -*- coding: utf-8 -*-
"""
trading_system/ai/lstm_predictor.py

LSTM 기반 시계열 주가 예측 모델
"""
...
class LSTMPredictor:
    ...
    def __init__(self, config=None):
        ...
        if not HAS_TENSORFLOW:
            self.logger.warning("⚠️ TensorFlow가 설치되지 않았습니다. LSTM 예측을 사용할 수 없습니다.")
            self.enabled = False
            return
        ...
        self.logger.info("🧠 LSTM 예측 모델 초기화 완료")
    ...
    def train(self, data: pd.DataFrame, epochs: int = 100, batch_size: int = 32) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"🎯 LSTM 모델 학습 시작 - 데이터: {len(data)}개")
    ...
    def predict(self, data: pd.DataFrame, days: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"📈 LSTM 예측 완료: {current_price:.0f} → {predictions_original[-1]:.0f} ({change_rate:+.1f}%)")
    ...
    def _calculate_prediction_confidence(self, predictions: np.ndarray, current_price: float) -> float:
        ...
        # 변동성이 낮을수록 신뢰도 높음 (0.8 ~ 0.95 범위)
        confidence = max(0.5, min(0.95, 0.95 - pred_volatility * 10))
        return confidence
    ...
    def save_model(self) -> bool:
        ...
        try:
            ...
            model_path = self.model_dir / f"lstm_model_{timestamp}.h5"
            scaler_path = self.model_dir / f"lstm_scalers_{timestamp}.pkl"
    ...
    def load_model(self, model_path: str, scaler_path: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ LSTM 모델 로드 완료: {model_path}")
            return True
