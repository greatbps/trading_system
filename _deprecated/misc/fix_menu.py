#!/usr/bin/env python3
# Fix the menu handlers file by inserting blocking code

with open('menu_handlers.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the line with the Panel call and insert blocking code after it
new_lines = []
for i, line in enumerate(lines):
    new_lines.append(line)
    
    if 'console.print(Panel("[bold green]종합 분석 (5개 영역 통합)[/bold green]"' in line:
        # Insert blocking code
        blocking_code = '''        
        # 기능 비활성화 - 사용자 요청
        console.print("[bold yellow]⚠️ 기능 사용 제한[/bold yellow]")
        console.print("\\n[bold red]🚫 종합 분석 기능은 현재 사용할 수 없습니다.[/bold red]")
        console.print("[yellow]💡 이 기능은 시스템 안정성을 위해 일시적으로 비활성화되었습니다.[/yellow]")
        console.print("[dim]   대신 다른 분석 옵션을 사용해 주세요:[/dim]")
        console.print("[cyan]   • 5번: 특정 종목 분석[/cyan]")
        console.print("[cyan]   • 6번: 뉴스 재료 분석[/cyan]")
        console.print("[cyan]   • 9번: AI 종합 시장 분석[/cyan]")
        console.print("\\n[dim]메인 메뉴로 돌아가려면 아무 키나 누르세요...[/dim]")
        
        # 사용자 입력 대기
        try:
            input()
        except EOFError:
            pass
        except Exception:
            pass
        
        return True
'''
        new_lines.append(blocking_code)

# Write back
with open('menu_handlers.py', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print("Menu handlers fixed!")