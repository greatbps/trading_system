...
class FundamentalAnalyzer:
    ...
    async def _analyze_financial_metrics(self, financial_data: Dict, stock_data: Any) -> Dict[str, Any]:
        """실제 재무 지표 분석"""
        self.logger.debug(f"FundamentalAnalyzer received financial_data: {financial_data}")
        try:
            # 점수 초기화
            ...
            # 수익성 (ROE) 분석
            roe_val = financial_data.get('roe_val') # 새로운 API에서 가져온 ROE 값
            if roe_val is not None:
                ...
                # 기존 PE, PBR 기반 ROE 근사치 분석 (데이터 없을 때만 사용)
                pe_ratio = financial_data.get('pe_ratio')
                pbr = financial_data.get('pbr')
                if pe_ratio and pbr and pe_ratio > 0:
                    ...
                    profitability_score = 50 # 데이터 없음
                roe_val = roe_approx if 'roe_approx' in locals() else None # 결과에 포함하기 위해
