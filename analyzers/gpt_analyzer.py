...
# -*- coding: utf-8 -*-
"""
trading_system/analyzers/gpt_analyzer.py

OpenAI GPT를 활용한 뉴스 및 감성 분석기 (Gemini 백업)
"""
...
class GPTAnalyzer:
    ...
    def __init__(self, config: Config):
        ...
        if api_key and api_key.startswith('sk-'):
            try:
                ...
                self.logger.info("✅ GPT API 클라이언트 초기화 완료")
            except Exception as e:
                self.logger.error(f"❌ GPT API 클라이언트 초기화 실패: {e}")
                self.api_available = False
        else:
            self.logger.warning("⚠️ GPT API 키가 설정되지 않음")
            self.api_available = False
    ...
    async def _call_gpt_api(self, prompt: str, max_tokens: int = 2000) -> str:
        ...
        try:
            response = await self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "You are a Korean stock market analyst. Always respond in Korean and provide accurate, detailed analysis."},
    ...
    async def analyze_market_impact(self, symbol: str, company_name: str, news_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"✅ GPT 시장 영향도 분석 완료 - {symbol}: {result.get('impact_level', 'UNKNOWN')}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ GPT 시장 영향도 분석 실패 ({symbol}): {e}")
            return self._get_default_market_impact()
    ...
    async def analyze_news_sentiment(self, symbol: str, company_name: str, news_data: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"📰 {symbol} 뉴스 {len(news_data)}개로 GPT 감성 분석 시작...")
            ...
            self.logger.info(f"✅ GPT 통합 감성 분석 완료 - {symbol}: 단기={result.get('short_term', {}).get('score', 50)}, 중기={result.get('mid_term', {}).get('score', 50)}, 장기={result.get('long_term', {}).get('score', 50)}")
            return result
            
        except Exception as e:
            self.logger.error(f"❌ GPT 통합 감성 분석 실패 ({symbol}): {e}")
            return self._get_default_sentiment()
...
"""
    ...
    def _parse_market_impact_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.warning(f"GPT 시장 영향도 분석 응답 파싱 실패: {e}")
            return self._get_default_market_impact()
        ...
        try:
            ...
            if json_match:
                ...
                return {
                    ...
                    'overall_summary': result.get('overall_summary', 'GPT 통합 분석 완료')
                }
            else:
            ...
            self.logger.warning(f"GPT 통합 감성 분석 응답 파싱 실패: {e}")
            return self._get_default_sentiment()
    ...
    async def analyze_comprehensive(
        ...
        try:
            self.logger.info(f"🤖 GPT 종합 분석 시작: {symbol}({name})")
            ...
            if response and response.strip():
                self.logger.info(f"✅ GPT 종합 분석 완료: {symbol}")
                return response
            else:
                self.logger.warning(f"⚠️ GPT 응답 없음: {symbol}")
                return self._get_fallback_comprehensive_analysis(symbol, name, stock_data)
                
        except Exception as e:
            self.logger.error(f"❌ GPT 종합 분석 실패: {symbol} - {e}")
            return self._get_fallback_comprehensive_analysis(symbol, name, stock_data)
...
"""
    ...
    def _get_default_sentiment(self) -> Dict[str, Any]:
        ...
        return {
            'short_term': {
                'score': 50,
                'summary': '뉴스 데이터 부족으로 중립적 전망 (GPT 분석 미실시)',
            ...
            'overall_summary': 'GPT 분석 불가 시 기본 중립 분석 사용'
        }
    ...
    def _get_fallback_comprehensive_analysis(
        ...
        fallback_analysis = {
            "buy_score": 50,
            "confidence": 30,
            "reasoning": f"{name}({symbol})에 대한 기본 분석입니다. GPT API가 사용 불가능하여 제한된 분석을 제공합니다.",
            "recommendation": "HOLD",
            "key_factors": [
                "GPT 분석 서비스 일시 중단",
                "기본 정보 기반 중립 평가",
    ...
    def model(self):
        """호환성을 위한 model 속성"""
        return "gpt-3.5-turbo" if self.api_available else None