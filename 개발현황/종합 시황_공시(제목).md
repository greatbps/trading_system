...
모의 Domain	모의투자 미지원					
URL 명	/uapi/domestic-stock/v1/quotations/news-title					
개요						
개요	"종합 시황/공시(제목) API입니다. 
...
Request Query Parameter	FID_NEWS_OFER_ENTP_CODE	뉴스 제공 업체 코드	string	Y	40	공백 
    FID_COND_MRKT_CLS_CODE	조건 시장 구분 코드 	string	Y	6	공백 
    FID_INPUT_ISCD	입력 종목코드	string	Y	12	공백: 전체, 종목코드 : 해당코드가 등록된 뉴스
    FID_TITL_CNTT	제목 내용	string	Y	132	공백 
    ...
    FID_RANK_SORT_CLS_CODE	순위 정렬 구분 코드	string	Y	2	공백 
    FID_INPUT_SRNO	입력 일련번호	string	Y	20	공백 
...
Example						
Request Example (Python)	"FID_NEWS_OFER_ENTP_CODE:
